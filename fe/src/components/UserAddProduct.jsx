...
function AddProductForm() {
  ...
  const addProduct = async (productData) => {
    ...
    const response = await axios.post('http://localhost:5500/products/', productData, config);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      productData.append('id_category', productCategory);
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto mt-8">
      ...
      <div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="product-category">
          Kategori Produk
        </label>
        <input
          className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
